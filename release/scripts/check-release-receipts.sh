...
#   package archive hash from a git worktree of the recorded commit and
#   compares (proves the npm tarball is reproducible from its source commit).
#
...
for receipt in release/receipts/*.json; do
  ...
  # git checkout of the recorded commit (a worktree, so npm sees the same tree
  # as the release) and compare.
  if [ "$MODE" = "--full" ]; then
    TMP="$(mktemp -d)"
    if git worktree add --detach "$TMP" "$commit" >/dev/null 2>&1; then
      real="$(
        cd "$TMP"
        # npm pack runs prepack (npm run build → tsc → dist/), so the worktree
        # needs its devDependencies first (the lockfile pins the same versions).
        npm ci --silent >/dev/null 2>&1 || true
        tb="$(npm pack --pack-destination "$TMP" --json 2>/dev/null | jq -r 'to_entries[0].value.filename')"
